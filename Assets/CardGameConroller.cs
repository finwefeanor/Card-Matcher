...
public class CardGameConroller : MonoBehaviour
{
    ...
    void CreateCards() 
    {
        ...
        for (int cardNumber = 0; cardNumber < totalCardNumbers; cardNumber++)
        {
            ...
            Card card = cardList[cardNumber].GetComponent<Card>();
    ...
    public void CardOpened(GameObject newCard) 
    {
        ...
        if (firstCard == null)
        {
            ...
            firstCard = newCard;
            firstCard.GetComponent<Card>().isCardLocked = true;
        }
        // when pressed second card
        else
        {
            // if value of two cards are same
            if(firstCard.GetComponent<Card>().value ==
                newCard.GetComponent<Card>().value)
            {
                Debug.Log("Cards are Same! Pisti!");

                // lock the second card
                newCard.GetComponent<Card>().isCardLocked = true;
            ...
            else
            {
                ...
                // unlocked first card
                firstCard.GetComponent<Card>().isCardLocked = false;

                // close two cards
                firstCard.GetComponent<Card>().CloseCard();
                newCard.GetComponent<Card>().CloseCard();
